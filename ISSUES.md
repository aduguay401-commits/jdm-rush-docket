...
### 2026-07-12 - Stage 2.5 Purchase Close-Out gate was never built

The lifecycle roadmap's Stage 2.5 (two-part signed+paid gate + Move to Delivery) had no UI, even though the first real customer already signed and paid. Built on feature/purchase-closeout (no schema — columns exist): a Purchase Close-Out section on the agent docket page showing agreement_signed + deposit_paid, a Mark/Unmark Deposit Paid action via the agent PATCH route (whitelist += deposit_paid only), and a Move to Delivery action (visible only when both green) that sets a new sold_in_delivery status via a dedicated endpoint with a server-side gate re-check and status-history write. sold_in_delivery added to every status label/line/progress/stripe/urgency map across agent, admin, activity feed, and customer surfaces (positive customer wording, never snake_case), buckets as Working with an "In delivery" line, and is hard-excluded from the follow-up cron (plus any agreement_signed docket). Pending gate + Reviewer/QA + merge.
