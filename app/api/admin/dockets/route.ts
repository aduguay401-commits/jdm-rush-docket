...
export async function GET(request: Request) {
  ...
  try {
    ...
    const query = supabase
      .from("dockets")
      .select(
        "id, created_at, status, customer_first_name, customer_last_name, customer_email, customer_phone, vehicle_year, vehicle_make, vehicle_model, destination_city, destination_province, budget_bracket, timeline, additional_notes, admin_notes, is_flagged, is_paused, paused_until, lost_reason, estimated_deal_value, marcus_questions_count:marcus_questions(count), customer_questions_count:customer_questions(count), marcus_questions(id, question_text, answer_text, answered_at, created_at), auction_research(*), private_dealer_options(*), follow_up_sequences(*), email_log(*), docket_status_history(*)"
      )
